...
- (void)loadView
{
	const CGFloat factor = (IS_IPAD()) ? 2.2f : 0.90f;
	...
	if(IS_IPAD())
	{
		if(usesAdvancedRemote)
			_landscapeFrame = CGRectMake(140, 15, mainViewSize.height - 140, mainViewSize.width);
		else
	...
	else
	{
		if(usesAdvancedRemote)
		{
			...
			_landscapeFrame = CGRectMake(85, 62, mainViewSize.height - 85, mainViewSize.width - 80);
		}
		else
	...
	{
		if(usesAdvancedRemote)
			_landscapeNavigationFrame = CGRectMake(222, 222, 360, 285);
		else
